...
"""
.. module:: gen_t_python_events_pred.py
    :synopsis: This module generates data to be use in the CHANNEL_CHANGE prediction problem.
               It also calculates all th variables that use historical data only using train data.
...
if __name__ == '__main__':
    ...
    df = util.get_df_from_conn('SELECT * FROM V_EVENTS ORDER BY ORIGIN_TIME;', 'local')

    print 'Calculating features...'

    # Separating train from test
    df['DATA_SET'] = df.ORIGIN_TIME.apply(util.split_train_test_val_det)
    ...
    # 1. by program
    df = df.join(df[df['DATA_SET'] == 1].groupby(['PROGRAM_NAME'])['DURATION_MIN'].sum(), on='PROGRAM_NAME', rsuffix='_SUM_PROGRAM_NAME')
    df['RANK_PROGRAM_NAME'] = df['DURATION_MIN_SUM_PROGRAM_NAME'].rank(method='dense', ascending=False).to_frame()

    # 2. by channel
    df = df.join(df[df['DATA_SET'] == 1].groupby(['CHANNEL_NAME'])['DURATION_MIN'].sum(), on='CHANNEL_NAME', rsuffix='_SUM_CHANNEL_NAME')
    df['RANK_CHANNEL_NAME'] = df['DURATION_MIN_SUM_CHANNEL_NAME'].rank(method='dense', ascending=False).to_frame()

    # 3. by genre
    df = df.join(df[df['DATA_SET'] == 1].groupby(['CHANNEL_GENRE'])['DURATION_MIN'].sum(), on='CHANNEL_GENRE', rsuffix='_SUM_CHANNEL_GENRE')
    df['RANK_CHANNEL_GENRE'] = df['DURATION_MIN_SUM_CHANNEL_GENRE'].rank(method='dense', ascending=False).to_frame()

    # Creates a rank of the CHANNEL_CHANGE programs by PROGRAM_NAME / CHANNEL_NAME / CHANNEL_GENRE
    # 1. by program
    df = df.join(df[df['DATA_SET'] == 1].groupby(['PROGRAM_NAME'])['CHANNEL_CHANGE'].sum(), on='PROGRAM_NAME', rsuffix='_SUM_PROGRAM_NAME')
    df['RANK_PROGRAM_NAME_CHANNEL_CHANGE'] = df['CHANNEL_CHANGE_SUM_PROGRAM_NAME'].rank(method='dense', ascending=False).to_frame()

    # 2. by channel
    df = df.join(df[df['DATA_SET'] == 1].groupby(['CHANNEL_NAME'])['CHANNEL_CHANGE'].sum(), on='CHANNEL_NAME', rsuffix='_SUM_CHANNEL_NAME')
    df['RANK_CHANNEL_NAME_CHANNEL_CHANGE'] = df['CHANNEL_CHANGE_SUM_CHANNEL_NAME'].rank(method='dense', ascending=False).to_frame()

    # 3. by genre
    df = df.join(df[df['DATA_SET'] == 1].groupby(['CHANNEL_GENRE'])['CHANNEL_CHANGE'].sum(), on='CHANNEL_GENRE', rsuffix='_SUM_CHANNEL_GENRE')
    ...
    df = df.join(df[df['DATA_SET'] == 1].groupby(['BOX_ID', 'PROGRAM_NAME'])['CHANNEL_CHANGE'].sum(),
                 on=['BOX_ID', 'PROGRAM_NAME'], rsuffix='_PER_USER_TEMP_P')
    ...
    # Get the sum of change channel per user
    df = df.join(df[df['DATA_SET'] == 1].groupby(['BOX_ID', 'CHANNEL_NAME'])['CHANNEL_CHANGE'].sum(),
                 on=['BOX_ID', 'CHANNEL_NAME'], rsuffix='_PER_USER_TEMP_C')
    ...
    util.write_table_to_my_sql(df, 't_python_events_pred', 'ALTER TABLE T_PYTHON_EVENTS_PRED ADD PRIMARY KEY (ID)'
                               , 'local')
