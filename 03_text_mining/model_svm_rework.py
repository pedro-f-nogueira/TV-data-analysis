...
.. module:: model_svm.py
        :synopsis: This module used a SVM to determine several categories using program descriptions
                   ...
                   Here I combined the movies and series category and removed the others category in order
...
if __name__ == '__main__':
    ...
    df = util.get_df_from_conn('SELECT CASE WHEN MASTER_CATEGORY = \'FILMES\' OR MASTER_CATEGORY = \'SERIES\' '
                               'THEN \'FILMES E SERIES\' '
                               'ELSE MASTER_CATEGORY '
                               'END AS MASTER_CATEGORY, '
                               'CLEAN_DESCR '
                               'FROM T_TRAINING_TEXT_MODEL '
                               'WHERE MASTER_CATEGORY != \'OUTROS\' '
                               , 'local')
    ...
    # Lets apply tfidf to the output of countvectorizer
    ...
    classifier_rbf = OneVsRestClassifier(SVC(kernel='linear'))
